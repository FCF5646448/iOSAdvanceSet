...
### UI相关
#### 图像显示原理
iOS的所有视图控件都是一个UIView，而根据“单一职责”的设计原则，真正负责UIView的显示部分是CALayer。CALayer有一个id类型属性叫contents，它实际对应着一个CGImageRef（位图）。也就是说我们看到的最终显示都是位图。
**那位图是怎么生成的**？
一张位图的生成是有CPU和GPU合作完成的。在我们代码构建完一个UIView后。
