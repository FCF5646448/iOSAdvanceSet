...
二就是使用一个子线线程NSThread定时查看主线程是否能够响应，如果主线程能够在时间阈值内响应。则说明没有卡顿，如果响应的时间超出了





卡顿定位：刚才说过，FPS是没法定位到具体函数的。具体方法：一是使用Instrument的Timer Profile来分析代码的执行时间，基本可以定位到具体的函数。二就是对第二种方法

4、回答如3题的第二种检测方案。只能说主线程放置CADisplayerLink只能是一种择中的方案。
5、使用RunLoop做卡顿优化，是指在RunLoop休眠的时候进行一些异步绘制、异步解码的操作，比如ASyncDisplayerKit的做饭一样的。其实原理跟第二种检测方式是一样的。
