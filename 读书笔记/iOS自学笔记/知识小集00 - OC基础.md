...
block对于不同的数据类型，有不同的截获方式。全局变量、静态局部变量都是存储在数据区，所以在block内对其进行修改，也可以成功；auto对象类型，是连同所有权修饰符一起截获，说白了其实就是改变了引用计数而已，最终也是通过block内部的指针找对对应的对象，进行修改，也可以成功。那如果我要对auto基础数据类型进行修改，那是否能够实现？答案是不能。
所以这就出现了 _ _ block。
什么是 _ _ block? 
**_ _ block其实是一个存储区域说明符，它可以指定变量值存储到哪个区域中**。所以_ _ block可以解决block内部无法修改auto基础数据类型的问题。但是注意：_ _ block不能修饰全局变量、静态变量（因为这两个变量存储在数据区，数据区里的东西，编译完成后就不能轻易动）。

具体实现：
添加了 _ _ block修饰符的变量最终会被封装成一个结构体：
结构体中包含一个与变量同名的成员变量及其值；结构体中还包含一个 **_ _ forwarding指针，初始化指向自身地址**；结构体中含有isa指针。也就是说使用了 _ _ block修饰的变量最终变成了对象。
