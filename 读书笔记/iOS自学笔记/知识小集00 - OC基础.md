...
* 注意几个关键词：
  ...
  * 类对象的superClass指针指向父类对象，元类对象的superClass指针指向父类的元类，根元类的superClass指针指向NSObject，NSObject的superClass指针为nil；
...
* 注意：
  ...
  * commonMode是一种技术解决方案。RunLoop启动时只能指定一个Mode作为当前的currentMode，如果要切换Mode，只能先退出当前Mode，重新选择一个Mode进入。而commonMode是一种伪模式，它是指标记了Common属性的Mode，默认被标记了Common属性的Mode只有DefaultMode，TrackingMode。也可以使用CFRunLoopAddCommonMode 方法向 Common Modes中添加自定义 modes。
    ...
    CommonMode就是为了解决一个RunLoop只能有一个Mode而产生的技术解决方案。而一个mode一旦被标记为common属性的话，则该mode的名字字符串就会被放到commonModes集合中，当runloop的mode发送变化时，runloop会将commonModeItems中的Observer、timer、source同步到具有Common属性的mode中，这样就可以让一些事件可以同时在多个mode中工作。比如NSTimer，当其被添加进commonMode中时，它就可以在标记了common属性的DefaultMode和TrackingMode中同时运行。
...
局部变量截获问题总结一下：
  ...
  * 对于局部auto对象类型则是采用**连同其所有权修饰符一起截获**。如果是栈block，则无论修饰符是什么，都不会产生强引用（因为栈block随时都会释放，即使进行了强引用，那么block释放的时候，引用计数也会里面减一，所以就没有必要强引用）；如果是堆block，则会根据其所有权修饰符来确实是弱引用还是强引用。如果是强引用的话，则就可以保证被截获的对象肯定是在block之后被销毁的，所以对象在block内外的变化，都可以被同步到。因为最终操作都是通过指针找到对应的对象进行操作。其次这也存在内存管理相关的东西，所以这也就可能导致循环引用的问题。（说白了其实就是根据修饰符来改变了引用计数而已，如果最终没有被释放（强引用）最终也是通过block内部的指针找对对应的对象，然后进行操作）。
...
通过上面的分析，我们知道对全局block进行copy不会有任务反应，对栈block进行copy则会被拷贝到堆上，对堆block进行copy操作则会增加其引用计数（所以是个浅拷贝）。//（ 那实际上，对栈block进行copy操作时，在堆上也产生了一个一样的block。同时，在MRC下，对栈block进行copy操作就如同执行了一个malloc方法，所以同样存在内存泄漏的问题。这一段有待商榷，单纯的copy貌似不会产生 _ _ forwarding指针）。

#### --block：

问题：为什么会出现_ _ block？

block对于不同的数据类型，有不同的截获方式。全局变量、静态局部变量都是存储在数据区，所以在block内对其进行修改，也可以成功；**对auto变量进行修改，那是否能够实现？答案是不能**(我们如果在block里对auto类型变量进行**修改**，会直接报错)。其次我猜测是因为block里修改auto变量实际上就会触发到内存相关问题，首先对于auto基础数据类型，因为是值传递进来的，所以修改了block里面的值，就没法同步到block外面，所以编译器不允许；对于auto对象类型，在block里修改了对象（其实就是重新赋值），那就是在堆上开辟了一块新的内存空间的问题，所以两个对象无关如关联的话，那其实也是在block内外无法同步的，所以编译器同样不会允许。
所以这就出现了 _ _ block。
什么是 _ _ block? 
...
当auto变量变成结构体对象后，那么就肯定就出现内存管理的问题。__block 修饰的对象的内存是怎么管理的呢？

首先我们知道block对auto对象类型的变量的截获方式是**连通其所有权修饰符一起截获**，然后根据其修饰符来确定是强引用和弱引用。那其实这里也是一样的，使用_ _block修饰了的变量，最终变成了一个对象。那么block对这个对象也是连同其所有权修饰符一起截获， **block修饰其实就是强引用**。

不过这里的截获方式，也只是说block内部对auto变量的使用方式，可能会影响其引用计数。而auto变量变成了对象，也只是解决了对象可以在block内部进行修改的问题。但是auto变量始终是局部变量，它迟早会被释放掉，那如何既可以在block内部使用，也可以使得修改同步到任何一个引用了该变量的block里面呢？

答案就是将 _ _ block修饰的变量也一并被拷贝到堆上。也就是说在block被copy到堆上的同时，_ _ block修饰的变量也会一同不被拷贝到堆上。当block被dispose的时候，_ _block修饰的变量也会被执行release操作。
