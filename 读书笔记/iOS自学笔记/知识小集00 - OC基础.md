...
* OSSpinLock是自旋锁，自旋锁是忙等的锁，被访问资源被锁时，线程不会休眠，而是不停地循环等待，直到被锁资源被释放，所以自旋锁效率较高。但是存在优先级反转的问题：优先级低的线程先获取资源，优先级高的线程未获取到资源，但是优先级高的线程将被优先调度。 最终导致优先级更高的线程一直在等待解锁，但是却会一直占用CPU，优先级低的线程获取到资源却没有CPU去执行，而无法释放锁；最终导致相互等待的结果。所以自旋锁已经不再使用了；

* os_unfair_lock 用来替换OSSpinLock，但是它并非忙等的锁，等待的过程中也会处于休眠；iOS10开始使用；
...
* pthread_mutex(recursive) 递归锁；（因为递归锁需要处理）
