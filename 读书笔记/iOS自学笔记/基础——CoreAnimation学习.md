...
* 动画和屏幕上组合的图层实际上被一个单独的进程管理，而不是当前应用程序。这个进程就是**渲染服务**。当运行一段动画时，这个过程会被分离成4个阶段：
	* 布局：准备视图/图层的层级关系，设置图层属性；
	* 显示：图层寄宿图片被绘制，这里可能会调用drawRect:或drawLayer:inContext:；
	* 准备：Core Animation准备发送动画数据到渲染服务；
