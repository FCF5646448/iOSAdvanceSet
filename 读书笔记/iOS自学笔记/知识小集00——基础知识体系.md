...
* 每一帧画面的生成是由CPU和GPU合作完成的。
  CPU主要负责包括视图的创建、布局计算、文本绘制、如果是图片的话，还会在提交给GPU之前进行图片的**解码**。最后将CPU生成的位图，通过Core Animation提交给GPU。GPU的部分就是OpenGL渲染管线相关的工作。主要就包括**顶点着色**、**图元装配**、**几何着色**、**光栅化**、**片段着色**、**片段处理**等，最后将生成的内容放到帧缓存区中。等到VSync信号到来之前，视图控制器就会去帧缓存区进行提取将要显示的内容。总的来说，Core Graphics渲染部分由CPU完成，OpenGL部分由GPU完成。

  * CPU擅长复杂的操作，GPU擅长简单的并行计算。（CPU就像是一个大学生做高数，GPU就类似几千个小学生做加减乘除）
#### UI卡顿掉帧的原因

iOS设备显示器每绘制完一帧画面，复位时会发送一个VSync垂直同步信号，并且此时切换帧缓存区（iOS 是双缓冲）。然后视图控制器就会去帧缓存区提取显示内容，最终显示在屏幕上。

那如果一个VSync信号周期内，CPU和GPU没能协作完成提交，那么那一帧就会被丢弃，此时屏幕就不会刷新，仍然显示之前的内容。这样就造成了卡顿。

* 按照苹果60FPS的刷帧率，每隔16.7ms就会有一次VSync信号。

* 在VSync信号到来后，系统图形服务是通过CADisplayLink来通知APP的。

* VSync是垂直同步信号，表示一帧画面绘制完成。还有一个水平同步班信号HSync，表示一行像素绘制完成。

* iOS是双缓冲机制。也就是会先将渲染完成的一帧放入缓冲区，供视频控制器读取。然后开始渲染另一帧。

  问：如果不用VSync垂直信号，而只是一帧渲染完成就切换行不行？

  答：不行，因为视频控制器读取画面也要耗时，如果视图控制器读取到一半的时候，另一帧就渲染完成，那么就可能会出现两个画面，形成撕裂现象。

  所以现在GPU会等待VSync信号发出后，才开始新一帧的渲染和缓冲区的更新。只是可能会带来部分延迟的问题。
#### 卡顿优化
...
* 对象调整：主要是CALayer的一些属性的调整，比如frame、bounds等，它实际上是通过运行时resolveInstanceMethod为Layer临时添加一个方法，然后将对应属性保存到Dictionary里，同时通知Delegate、创建动画。 其次改变CALayer的一些**可动画属性值**时，会对模型层数据做动画，最后显示在展示层上，也多了一份数据的拷贝。所以对象(尤其是视图里的对象)的属性调整应尽量避免，尤其是视图层次，以及频繁地添加和移除视图。尽量提前计算好布局，在有需要时一次性调整对应的属性，不要多次修改属性。
...
* 图片优化：
  ...
  图片的解码是在提交给GPU之前，且发送在主线程。所以可以参考**SDWebImage提前进行解码操作**；

  图片的绘制：尽可能不要在系统绘制流程中会触发后备缓存的方法里且在主线程进行绘制，尽可能使用异步绘制。
...
* **离屏渲染**：
  ...
  * 在OpenGL中有两种渲染方式：当前屏幕内渲染和离屏渲染(在当前屏幕缓存区以外开辟一个新的缓存区进行渲染)。
    导致离屏渲染的原因是：(早期)，iOS绘制原理是根据画家算法，由远及近一层一层渲染。也就是说上一层必须要在下一层渲染完成之后才能渲染。但是iOS有一些属性是依靠上一层的渲染结果的，比如阴影、圆角、蒙板等，都是得在上一层的渲染结果拿到后，然后根据上一层的路径，去渲染对应的图层，最后插入该图层的下一层。所以应对这种情况，就得另外开辟缓冲区，将两个图层渲染整合完成后，再放到当前缓冲区内。这就是所谓的离屏渲染。
  * 离屏渲染消耗性能的原因包括：
    1.需要创建新的缓存区；
  ...
  * 哪些操作会触发离屏渲染？
    ...
    4.阴影。
  * 如何优化？
    1.shouldRasterize如果设置成YES，在触发离屏渲染的同时，会将光栅化后的内容缓存起来，下一帧可以直接复用，这可以提审很高的渲染性能。
    2.当遇到这些容易触发离屏渲染的情况的时候，可以考虑使用CPU渲染，也就是直接调用Core Graphics的API绘制来替代。

* **处理时机与Core Animation的Observe回调**：

  Core Animation在RunLoop中注册了一个优先级比较低的Observer，这个Observe监听了RunLoop的BeforeWaiting和Exit状态。当处理了UI时，比如创建和调整了视图层级、修改了frame、透明度、为视图添加了一个动画，或者手动调用了setNeedsDisplay、setNeedsLayout等方法。对应的CALayer就会标记为待处理，并被提交到一个全局容器里。 当Observe监听到通知后（注意：这里如果有动画的话，则是通过DisplayLInk不断唤醒RunLoop，使得有机会不断触发这个Observer），然后回调方法里就会将待处理的视图进行实际的绘制和调整。最后将合成的位图交由Core Animation。Core Animation则将位图提交给GPU，最后显示在屏幕上。

  * 实际上iOS渲染是由一个独立的进程负责——Render Server。具体流程：当CPU对图层进行打包后，app通过IPC发送给Render Server；Render Server执行OpenGL、Core Graphics相关程序，并调用GPU；GPU则在物理层完成渲染，最终通过Frame Buffer显示到屏幕上。
  * APP发送图层给Render Server的过程其实就是Core Animation的4个细分步骤。

    * Layout：视图构建，包括对LayoutSubviews方法的重载、addSubView方法等；

    * Display：视图绘制，这里仅仅指设置图元数据。

    * Prepare：一般是处理图像的解码和转换等操作。

    * Commit：将图层进行打包，发送给Render Server。
  * 其实动画也是基于Core Animation流水线完成的。比如UIView Animation动画
    * 调用UIView animation:xxx:方法
    * 在Animation Block里进行layout、diplay、prepare、commit等步骤。
    * Render Server根据Animation逐帧进行渲染。
...
* **UIView绘制流程**：
  ...
  * CALayer的后备存储Backing Store : 当我们调用CALayer的drawInContext方法时，layer会自动创建一块以layer大小成正比的内存区域，这个区域就是后备存储（backing store）。所以也最好不要随意重写drawRect:等相关方法。
  ...
  [最详细的UI绘制原理](http://hchong.net/2019/05/11/iOS%E5%BC%80%E5%8F%91UI-UI%E7%BB%98%E5%88%B6%E5%8E%9F%E7%90%86/)

### OC基础特性

#### NSMutableArray底层

NSArray是线性连续内存，和C语言的数组应该差不多。但是NSMutableArray是可以插入和删除的，那如何做到高效呢？插入和删除的时候，如何做到尽可能少的移动或者不移地其他元素？

NSMutableArray是在C语言的基础上进行过优化的结构数组。它使用了环形缓冲区来处理内存移动的损耗。

首先C语言数组的话，假设你在0位置插入一个元素，那么它会将后续元素都往后移动一位；那如果你删掉了第0位置的元素，那么后续元素又都会往前移动。

那NSMutableArray做了哪些优化？首先它使用了一个_ userd来记录已使用的数量；使用 _size来表示整个Array的大小，使用 _ offset来表示缓冲区里的数组的第一个元素索引。当最后一个下标存满了之后，它又会回到头结点进行存储。这样就形成了一个环形存储区域。

有了offset，可以在不移地其他元素的情况下进行头尾的删除操作。中间元素的插入和删除也是尽可能少地移动元素。

其次可以循环使用空的区域。

注：**为了使得中间元素的移除和插入更方便，其实可以使用环形链表来使用。但是iOS好像没有这么做，因为打印的数组元素的内存地址确实是连续的**。

[NSDictionary和NSMutableArray底层原理](https://blog.csdn.net/Deft_MKJing/article/details/82732833)

#### NSMutableDictionary底层

NSDictionary是对_CFDictionary的封装。它内部使用了两个指针数组分别存储keys和values。两个数组的长度肯定是一样的。

然后底层代码中key和value都是通过hash来进行查找的。所以可以理解为存储keys和values的两个数组都是哈希表。且对key进行哈希函数计算出来的下标既对应keys数组的下标又对应values数组的下标。

从哈希表的知识了解到。拉链法的查找时间复杂度更高，而字典的查找时间复杂度是O(1)，所以判断字典底层的哈希表使用的是开放定址法。

##### 哈希表

哈希表是通过key值来直接访问的数据结构。具体来说。它是通过一个散列函数对key值进行计算，然后将结果映射到表中的一个位置。这个表就是散列表。（所以可以认为散列表首先是一个简单的数组，然后散列函数计算key值得到的结果(**哈希值**)就是数组的下标。）

所以这个**散列函数**就非常关键。因为对不同的key值，所计算出来的**哈希值**有可能是相同的。这就是造成了**冲突**。

由此，散列表就不可能是单纯的数组了。它还需要处理冲突。散列表处理冲突的最主要的方法就是**开放定址法 和 拉链法**。

* 拉链法

  拉链法的做法是，先用一个数组直接存储**下标**，这个下标也就是哈希值。然后每一个下标后面对应一个链表，链表存储哈希值相同的key值及其内容。这样就能解决冲突问题

  比如：有一组数据[（10：aa），（11：bb）,（12：cc），（13：dd），（21：ee）,（22：ff），（24：gg）]。哈希算法取余。所以数组下标0对应：（10：aa）；下标1对应：（11：bb）、（21：ee）；下标2对应：（12：cc），（22：ff） ... 

  拉链法的问题就是：如果冲突比较严重的话，后续链表就会很长。链表一长也不利于查找，性能就大打折扣，极端情况下会退化成链表。

* 开放定址法：

  开放定址法，顾名思义就是哈希值与数组下标不一定一一对应的。当有冲突的时候，位置会有一定的偏移（也就是说位置是开放的）。

  比如：有一组数据[（10：aa），（11：bb）,（12：cc），（13：dd），（21：ee）,（22：ff），（24：gg）]。哈希算法取余，假设数组count是10。所以数组标0 ——（10：aa）；1——（11：bb）；2——（12：cc）；3——（13：dd）; 4  —— （24：gg）；5、6、7 空 ； 9 —— （21：ee） ; 8 —— （22：ff） 。

  开放定址法的问题是：数组的大小可能不够存储，所以就得进行扩容（当然拉链法也存在需要动态扩容的问题）。
...
* **+load VS +initialize**：
  ...
  * +initialize：当类第一次接受到消息时，也就是在第一次使用某个类的时候，那么它将会调用该类的+initialize方法。类似于懒加载的效果。
...
* 扩展： 扩展可以说是一个匿名的分类，但是它又不是分类，因为它是**编译时**决议的，生命周期随之宿主类，只有声明，没有实现，依附在宿主类中。主要的作用就是用来声明私有属性、私有方法、私有成员变量。不能为系统类添加扩展。

* 有多个Category的同名方法。如何获取到宿主类的最元素方法？
...
* 原子属性：atomic: 原子性; nonatomic: 非原子性

  atomic修饰的属性的读写方法里是进行加锁了的。老版本使用的是自旋锁（新版本有可能使用的是unfair_lock）。注意是setter和getter方法里都使用了锁。而且自旋锁是一把忙等的锁。所以这也导致属性的访问效率不是很高，所以就不建议使用atomic。

  其次原子性操作并不能完全保证线程安全。它只能保证读写方法的原子性。

  eg1：当一个线程在get或set时，另一个线程进行了release操作，那就可能会直接crash。这就不能说是线程安全的。

  eg2：如果是一个可变数组的话，只能保证数组的读写方法的原子性，但是对数组里元素的操作是没法保证原子性的。（其次 atomic非常耗性能。一般我们保证可变数组的线程安全的最直接方式是使用信号锁，如果要实现多读单写则要使用dispatch_barrier_async）

  [自旋锁和atomic](https://juejin.cn/post/6844903850269212685)
...
* 几个重要的点：
  * 1、class_rw_t里的二维数组类型有protocols、properties、methods。在runtime的时候，class_data_bits_t**会先将ro里的方法、属性、协议拷贝到rw对应数组中去**，然后再添加category里的方法、属性、协议。也就是说rw中的二维数组不止分类添加的，也包括类原类的方法、属性、协议。其次category添加对应方法时会添加到二维数组的前面，所以方法查找时会先查找category里的方法，再查找原理里的方法。而rw里没有成员变量，成员变量只存放在ro里，而class_ro_t是指只读的相关数据，也就是说已编译好的对象是无法在通过分类添加成员变量的（其实在编译期间类的内存大小已经确定，也不允许轻易更改内存）。在class_rw_t的方法列表中查找时，对于已排到的使用二分法查找，对于没有排序的直接遍历查找。
...
* main函数的AutoreleasePool有什么作用，可否直接去去掉？
  ...
  其实在主线程的RunLoop开启的时候就在主线程RunLoopEntry的时候就创建了一个AutoreleasePool。并且执行了push操作，所以main函数里的去不去掉应该问题都不大。其次子线程不开启RunLoop，其实也是一样的，因为主线程有AutoreleasePool。而且无论什么哪里的AutoreleasePoolPage，其实都是添加进同一个双向链表里面。只是page是区分thread的。
...
* 线程
  **是指进程中一个单一顺序的控制流，是程序执行的最小单位**。进程可看做是线程的容器，一个进程可以并发多个线程，每条线程单独执行不同任务，**各个线程之间共享进程的内存空间（包括代码段、数据段、堆等）**，所以线程也不是越多越好，开辟子线程也就意味着消耗更多的内存；其次线程之间的上下文切换也会开销很大。
...
- 线程调度
  ...
  在单个处理器处理多个线程时，操作系统会让这些线程轮流执行。这些线程的执行过程则是根据线程的状态进行调度。这个叫做并行。
  ...
  - 等待（阻塞）
    ...
    处于运行中的线程有一个时间片的运行时间。刚创建的线程，不会立即执行，会先进入就绪队列；当线程被分配到CPU后，线程进入到运行状态；在运行期间，如果碰到非常耗时的操作，则线程会被挂起进入阻塞状态，以让CPU可以执行其他任务。
...
* 任务分为**同步**执行和**异步**执行：
**同步和异步主要影响会不会开启线程**。同步执行是在当前线程中**立即**执行，异步执行会在线程池中获取一个新线程进行执行，如果线程池中没有则会创建一个新的线程执行；**但是如果是在main队列中，无论是同步还是异步都不会开启新的线程，都是在主线程中执行**。
...
// 分析：最外面也是主线程，里面异步执行也是主线程，所以第一：不会开启新的线程。其次，所有任务(block)都添加到了主队列里，主队列是串行队列。所以添加进来的任务(block)必定是一个一个执行。所以打印结果是： 0、4 (sleep1s)、2、1 (sleep2)、3
    ...
    NSLog(@"4");
// 分析：最外层也是主线程，1、3也是添加到主线程。所以同理，都是在主线程上执行。其次任务block都是添加到主队列，主队列是串行队列，所以任务block都是一个一个派发。所以打印结果：0、2、4、1、3
    
// 0、2、1、4、3    0、1、2、3、4   0、1、2、4、3
...
// 分析：最外层是主队列。然后在一个串行队列中添加两个异步执行的任务。所以第一，主队列中的代码肯定是按顺序执行，也就是0、2、4的顺序是不会变的。第二，串行队列中的任务block也是一个一个派发，所以即使是异步执行（开新线程），但是任务是按顺序执行，所以1、3的执行顺序不会变。所以会有以下打印结果：0、1、2、3、4	0、1、3、2、4	0、1、2、4、3	0、2、4、1、3	0、2、1、3、4	0、2、1、4、3。
    
```
##### 全局队列优先级
创建全局队列时，有一个参数是优先级的标识，所以如果要在并行队列中，让任务先执行，这可以通过设置这个优先级来达到目的（但是这里要注意，先执行并不一定是第一个执行完，它只能保证开始的执行顺序而已）。它的优先级包括：high、default、low、background。在磁盘IO操作的时候，CPU可能会处于等待IO完成的状态，导致资源浪费，如果使用background的话，就可以实现throllter（节流）的效果，将IO启动时间延时，将CPU用于处理其他更重要的任务。
...
* **队列**：
  ...
  * NSOperationQueue通过设置最大并发量maxConcurentOperationCount来确定其实串行还是并发。默认值是-1，表示会根据系统条件设置最大值。
* 如何实现多线程：maxConcurrentOperationCount 大于1就是多线程操作
...
* 为什么自旋锁性能更强？
  ...
  因为自旋锁会在等待的过程中一直runing，减少了线程之间的上下文切换的消耗。

* 如果atomic只对写加锁，不对读加锁，会有什么问题？

  会有问题。像我们做多读单写时，因为写的过程中用栅栏拦截了读操作。但是如果只对写进行加锁，那读不加锁的话，读取到的数据就可能存在问题。因为有线程一直在进行写操作。所以实际上要对读写都进行加锁，而且是同一把锁。

[iOS锁](https://zhuanlan.zhihu.com/p/82300525)
...
* 3、子视图中，若能响应，则自底而上传递，若其子视图不响应，则自身是最合适的响应者。若不能响应，则传递给其同级的前一个兄弟视图；
...
* 客户端先向server发送一个SSL版本及支持的加密算法和随机数C；
* server会返回一个选定的加密算法、随机数S、server端证书。（证书包含域名、发行方、有效期、公钥、签名等信息）
* 客户端接收到后会先验证server证书，然后通过C、S、预组秘钥组装成会话秘钥；之后通过server端公钥对预组秘钥进行加密发送给server端；（验证过程就是通过证书里的相关信息推导出它的合法性）
* server端则通过私钥解密预组秘钥，然后通过C、S、预组秘钥组装会话秘钥。然后向客户端发送一个Finished消息，表示TLS握手完成。



简单来说：使用HTTPS实际上是经历了7次握手。3次TCP握手+4次TLS握手。5个RTT（RTT指报文往返时间）

所以HTTP2（因为使用HTTPS）的延时性就会非常高。所以后续的HTTP3貌似就是使用QUIC协议。（下文再说）

[HTTPS的7次握手和9倍延时](https://draveness.me/whys-the-design-https-latency/)



Q：HTTPS都使用了哪些加密手段？
混合加密：对称加密和非对称加密。非对称加密在公私钥中使用到，传输过程中则是使用对称加密。
Q：什么是非对称加密和对称加密？
非对称加密包含两个概念：公钥、私钥。加解密使用的钥匙不一样的。用公钥加密，就得用私钥解密；私钥加密就得用公钥解密。（RSA）
对称加密：加解密用的是同一个秘钥。（AES）

Q：HTTPS的验证证书怎么做的？
首先服务器返回的证书也是CA机构颁发的证书。所以证书里面会包含CA颁发机构、有效期、公钥、服务器域名、签名等信息。
验证过程：
客户端根据证书信息链，先确定证书是否为可信任证书，并且根据该证书查找到根证书信息，然后反向推导出当前CA证书的公钥等信息。拿到公钥等信息后与证书里的签名进行解密。然后与服务器返回的信息做比较，如果结果一致，那么就是合法的。
...
Q：怎么防止中间人攻击
* 方案1：进行网络请求的时候，判断是否设置了代理i，如果设置了代理，就不允许访问。

* 方案2：客户端本地做证书校验。主要是对域名进行校验。当然了，代价会很大，因为很多公司的域名包含子域名。所以要校验域名就得使用通配符域名。

* 方案3：使用证书。

Q：HTTPS是绝对安全的吗？为什么

​	不是，如果是用户主动运行证书使用，那么也是可以被中间人抓包的。

[HTTPS 抓包原理](http://www.javaobj.com/2020/05/https-security/)
[iOS如何防止HTTPS抓包](https://www.jianshu.com/p/4682aecf162d)


#### HTTP1.x和HTTP2的主要区别
* HTTP2采用二进制帧格式，HTTP1是文本格式。二进制格式更健壮；
...
**快速重传、快速恢复**：是指在达到拥塞时，回到新的门限值，以线性增长的方式发送报文，而不经过前面指数增长的慢开始阶段。



* 最新的拥塞控制协议使用了**BBR拥塞控制协议**

  [BBR拥塞控制算法原理](https://www.taohui.pub/2019/08/07/%E4%B8%80%E6%96%87%E8%A7%A3%E9%87%8A%E6%B8%85%E6%A5%9Agoogle-bbr%E6%8B%A5%E5%A1%9E%E6%8E%A7%E5%88%B6%E7%AE%97%E6%B3%95%E5%8E%9F%E7%90%86/)



Q：为啥有了流量控制还需要有拥塞控制？

因为流量控制只能控制客户端和服务器两台主机的问题。没法管理链路层、物理层相关的流量问题。所以需要在流量控制的基础上加上拥塞控制。
...
我们知道使用TCP协议存在建立和释放连接。另外TCP的各种维持可靠性、按序到底等数据。所以协议使用势必会影响到一定的性能问题。

**QUIC协议是基于UDP实现的。它不存在三次握手。然后QUIC优化了连接建立的握手延迟，同时在应用程序层面上实现了TCP的可靠性、TLS的安全性和HTTP2的并发性。它只需要用户端和服务端的应用支持QUIC协议就可以实现**

[QUIC协议](https://www.cnblogs.com/chenny7/p/11661962.html)
...
* 最新的DNS技术有一个叫做DNSSEC（DNS安全扩展）的东西。

  简述：DNSSEC通过一个密钥对DNS数据进行签名。然后公钥会被公开发布。所以如果DNS被篡改了的话，使用公钥解密出来的东西就会跟原始数据不一样，DNS服务器就不会返回数据给用户。


#### 其他问题

Q：一个TCP可以发多少个HTTP请求？
在HTTP/1.0中，服务器在结束一个HTTP响应后，就会断开TCP链接。不过当时也可以把Connection字段设置成keep-alive来开启持久连续。 
在HTTP/1.1后，Connection默认就是开启持久链接了。不会结束一次请求就断开。
所以一个TCP请求可以发送多个HTTP请求。

Q：一个TCP链接中，HTTP请求发送可以一起发吗？(比如一起发3个请求，再3个响应一起接收)

HTTP/1.1存在一个问题。单个TCP链接在同一时刻只能处理一个请求。意思就是两个请求的生命周期不能重叠。
不过HTTP/1.1提供看Pipelining的处理方式：一个支持持久链接的客户端可以在一个连接中发送多个请求(不需要等到任意请求的响应)。但是收到请求的服务器必须按照请求收到的顺序发送响应。(这么做的原因可能是浏览器没法根据响应结果来判断响应是对应哪一个请求的)
但是这样会存在一个问题，加上两个连续的请求。当第一个请求处理非常耗时时，后续请求就得等待。

所以HTTP/2 提供了Multiplexing多路复用特性。是可以做到在一个TCP连接中同时完成多个HTTP请求。

Q：HTTPS是否需要每次都建立SSL链接
在持久链接的情况下，TCP不需要重新建立链接，SSL自如也不需要重新建立连接。

Q：浏览器对应同一Host建立TCP链接数量是否有限制？
假设一个页面有几千张图片，而且有很多图片是处于同一Host下。那么肯定不会同时建立几千个TCP连接。
Chrome貌似允许对同一Host建立6个TCP连接。不同的浏览器也是不一样的。

[TCP和HTTP面试题](https://www.jianshu.com/p/aaba68b87416)
...
#### socket链接建立
建立Socket链接至少需要一对套接字，分别运行于服务端和客户端，步骤：
* 服务器监听 
	服务器开启，始终等待，实时监听客户端请求链接
* 客户端请求
	客户端发出请求，指定服务端Socket的地址和端口号
* 链接确认 
	当服务器Socket监听到客户端Socket的链接请求时，做出响应。建立一个新的进程，把服务端Socket的描述发给客户端，该描述得到客户端确认后就可以建立起Socket链接。而服务端则继续监听其他请求。
...
CALayer默认修改属性支持隐式动画的，在给UIView的Layer做动画的时候，View作为Layer的代理，Layer通过actionForLayer:forKey:向View请求相应的动画行为。

Layer内部维护着三份layer tree。分别是presentationLayer tree（动画树）、modelLayer tree（模型树）、renderLayer tree（渲染树）。在做动画时，我们修改动画属性，在执行动画的其实是presentLayer属性值，而最终显示在界面上的其实是ModelLayer。
...
[iOS锁详解](https://juejin.im/post/6844904167010467854)
				...
				1. * [iOS即时通讯](https://www.jianshu.com/p/2dbb360886a8)