...
* 多态：父类类型指向子类对象就是多态。在OC中使用runtime实现多态，在C++中，使用虚函数表实现多态。
	* 上面类中说到，类内存中前8个字节是用来存储来的类型信息，这8个字节实际指向另一个新的内存区域，这个内存区域的内容存储的是类的类型信息及类的方法地址。当多态的时候，每个类的类型信息会先存储在全局区，运行过程中类的前8个字节根据实际情况再来确定指向哪个类的类型信息区域。也就是使用的是C++的虚函数表的方式。
