...
* 每一帧画面的生成是由CPU和GPU合作完成的。
  当视图树发生变化时,会在Layer上标记，然后在runloop一个循环周期将要结束的时候，CPU会开始计算它的下一帧要显示的内容。包括对象的创建、布局计算、文本计算与渲染、如果是图片的话，还会在提交给GPU之前进行图片的**解码和绘制**。最后将CPU生成的内容，在**Core Animation的Observe回调**的时候合并提交给GPU。GPU的部分就是OpenGL渲染管线相关的工作。主要就包括**顶点着色**、**图元装配**、**几何着色**、**光栅化**、**片段着色**、**片段处理**等，最后将生成的像素点放到帧缓存区中。等到VSync信号到来之前，视图控制器就会去帧缓存区进行提取将要显示的内容。
#### UI卡顿掉帧的原因
...
* **异步绘制**：UIView被触发了setNeedsDisplay方法后，会在CALayer的setNeedsDisplay上打上一个脏标记。在下一次RunLoop即将结束的时候，CALayer才会调用display方法，如果实现了layer.delegate的display方法，则就进入了异步绘制入口。然后在代理方法里调用Core Graphics的API进行bitmap的绘制，最后回到主线程进行赋值
```OC
- (void)display{
...
* **Core Animation的Observe回调**：Core Animation在RunLoop中注册了一个优先级比较低的Observer，这个Observe监听了RunLoop的BeforeWaiting或Exit状态。也就是说在RunLoop即将进入休眠或退出时，Observe就会得到通知，然后回调方法里就会将CPU的合成的中间态的内容全部提交给GPU。如果是动画的话，还会通过DisplayerLink多次触发相关流程。
