...
```
**注意**：
Dispatch_semaphore在使用的过程中，如果对其赋新值或者将其置为nil，都会导致其Crash。
```
...
```
这是因为在libdispatch中，当我们创建信号量的时候，会使用一个dsema_value保留其当前值，使用dsema_orig保留其初始值。在执行dispatch_semaphore_wait是会对dsema_value进行-1操作，当执行dispatch_semaphore_signal时会对dsema_value进行+1操作。当对信号量进行置nil或者重新赋值的话，会先触发dispatch的disponse函数，在disponse函数的内部，当dsema_value小于dsema_orig时，会直接调用CRASH让程序崩溃。

https://juejin.im/post/5cc819cfe51d456e4869548d

#### Operation：
NSOperation是苹果在GCD的基础上封装的更面试对象的多线程方案。它使用两种概念：**操作Operation**和**队列OperationQueue**来实现多线程。将操作添加进队列中实现多线程。
**操作**：
...
**队列**：
	...
	* NSOperationQueue只提供了两种不同的队列：主队列和自定义队列；
##### NSInvocationOperation
NSInvocationOperation是不使用block方式的操作，同样的，单独的NSInvocationOperation只在当前线程执行。
```
...
```
##### NSBlockOperation
同样的，单独地使用NSBlockOperation也只是在当期线程执行
```
...
```
但是，NSBlockOperation还提供了一个**addExecutionBlock**来添加额外的操作。使用了addExecutionBlock的话，那所有的操作都可以在不同的线程中同时并发执行,包括NSBlockOperation。也就是说NSBlockOperation是否开启多线程一般取决于操作个数，但是开启的线程数是由系统决定的。
```
- (void)useBlockOperationWithAddExecutionBlock {
    NSBlockOperation * bop = [NSBlockOperation blockOperationWithBlock:^{
       [Thread sleepForTimeInterval:20]; 
    }];
...
```
* 并行Operation：
并行的Operation需要重写以下方法
	* start:和上面的main函数一致，将任务操作代码放里面，注意，任何时候都不用在这里调用父类的start方法。
	* isExecuting:是否正在执行，需要手动调用KVO方法来进行通知
	* isFinished:是否已完成，需要手动调用KVO方法来进行通知
	* isAsynchronous:是否并发执行
```
@interface CustomAsyncOpretion : NSOperation
@end

@interface CustomAsyncOpretion(){
    BOOL executing;
    BOOL finished;
}
@end

@implementation CustomAsyncOpretion
