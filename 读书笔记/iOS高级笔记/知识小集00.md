...
#### --block：
什么是 _ _ block? 
添加了 _ _ block修饰符的变量最终会变成一个结构体，结构体中含有isa指针。也就是说 使用了 _ _ block修饰的变量最终变成了对象。但是在**ARC和MRC下是有区别**的。**在MRC下使用 _ _ block修饰的变量，不会增加对象的引用计数；在ARC下，使用 _ _ block修饰的变量，会增加其引用计数**。所以就有一道痕经典的面试题:
...
```
一般我们对block的截获变量进行赋值操作时，需要先对其进行__block。
#### block的copy操作：
...
**点睛**：
说白了，block就是对象，block调用就是函数调用。所以同理，block也就是函数调用。既然是函数调用，那它肯定就存在作用域、栈帧等问题。所以从内存的角度来分析。一个GlobalBlock，相当于没有截获任何外部变量，也就是说没有传参，所以实际上不需要分配栈空间，也不存在变量的作用域问题；一个GlobalBlock使用了外部变量，就相当于需要传参了(局部基础数据类型使用值传递，局部对象类型使用引用传递)，所以就需要开辟栈空间来管理这些内存，所以就变成了StackBlock。但是由于栈空间在函数执行完毕之后也会被回收掉，所以如果将其赋值给某些__strong类型的属性block（或者其他操作），则得将其拷贝到堆中，这时就变成了堆block。
同样的，由于局部基础数据类型是值传递，所以就导致在block之后的修改对block里面不会有影响。所以要达到这个目标，就得使用__block，__block将基础数据类型变成了对象类型,变成了对象类型后内存自然就在堆区，然后就可以使用引用传参的方式进行传递，所以通常要对变量进行赋值，则就得用__block修饰，导致的结果就是变量变成了对象后被拷贝到了堆区。
...
**Cookie**主要是用来记录用户状态，区分用户；状态主要保存在客户端；主要操作就是第一次请求时，server端生成一个Cookier放到响应头部字段，然后客户端接收到之后进行存储，之后每次请求都在请求头部字段里带上Cookier。server端通过Cookier来判断用户登录状态。所以一般传输过程中也需要对Cookie进行加密，设置Cookie为HTTPOnly防止跨站脚本攻击。
**Session**也是主要用来记录用户状态，区分用户；状态主要存放在server端。它实际也是在客户端第一次请求的时候会生成一个SessionID，然后放到响应头部字段的set-Cookier的头部字段，然后客户端之后的每一次请求都在头部的cooiker带上SessionID字段。server端通过SessionID来验证是否是真实用户。
