...
设计模式主要包含设计原则和设计模式。设计模式就是实现了设计原则，达到了代码复用和可维护性。设计模式又包含创建型模式、结构型模式、行为型模式。
#### 设计原则
* **单一职责原则**：一个类只承担一个职责。比如UIView和CALayer的职责分离；
* **开闭原则**：对类、模块、函数尽可能扩展，而不是修改，如果要修改也尽量用继承或组合的方式来修改。比如category给元类添加方法；
* **里氏替换原则**：是指子类可以在任意地方替换父类的实例，而不会导致错误。换句话说，子类尽可能不要复写父类的方法，而是扩展其他的方法；
* **接口隔离原则**：是指定义接口时，尽量不要定义不必须使用的方法，比如说OC中定义protocol时，尽可能不要定义@optional的方法；其次定义的接口尽量细化，做到功能单一；整个接口方法也不宜过多，避免太臃肿；
* **依赖倒置原则**：指高层模块不应该依赖低层模块，二者都应该依赖抽象；抽象不应该依赖细节，细节应该依赖抽象。比如，面向协议编程、泛型的使用；
* **迪米特法则**：尽可能实现高内聚、低耦合；比如项目中路由的使用，MVVM中viewmodel的使用；
* 组合/聚合复用原则：尽量使用合成/聚合，尽量不使用继承；
#### 常用设计模式
##### 创建型模式
* 抽象工厂模式：
* 建造者模式：
