...
* **操作**：
也就是任务，对应GCD中的Block里的那部分代码。在NSOperation中，我们可以使用它的子类来定义操作：NSInvocationOperation、NSBlockOperation、自定义子类。
* **队列**：
队列是用于存放操作的队列。
它的执行顺序不同于GCD的先进先出的原则，而是根据状态来执行。它的状态包括4个状态：isReady(就绪)、Executing(运行中)、finished(完成)、Cancelled(取消)。然后队列会根据操作之间的依赖关系来确定哪个操作进入**准备就绪的状态**，其次操作的**开始执行顺序**由操作之间的优先级决定。
NSOperationQueue通过设置最大并发操作数来确定是串行还是并发操作；
NSOperationQueue提供了两种不同类型的队列：主队列和自定义队列。主队列运行在主线程之上，自定义队列运行在后台。
* 如何实现多线程
...
```
#### 自定义子类
自定义的NSOperation的子类可以分为**并发**和**非并发**两种。
* 非并发的子类
非并发的子类无需控制操作的执行状态，只需要重写main函数就可以：
```
...
- (void)main {
    @try{
        //提供一个变量标识，来表示需要执行的操作是否完成
        BOOL isFinished = NO;
        //while用来不断地判断操作是否执行完成或是否被取消了
...
要实现并发的子类则要复杂很多，因为要自己管理操作的状态，所以必须重写以下方法：
	...
	* main：非必须，因为可以直接将操作放到start方法里。但是通常会在这个方法里使用独立的自动释放池与别的线程区分开；
...
```
@interface Custom2Operation : NSOperation
- (Bool)beginOperation:(NSOperation *)op; //执行这个方法开启操作，而不是调用start；
@end

@interface Custom2Operation(){
    BOOL excuting; //执行中
    BOOL finished; //已完成
}
@end
@implementation Custom2Operation
- (instanceType)init{
    self = [super init];
    if (self){
        excuting = NO;
...
- (Bool)beginOperation:(NSOperation *)op {
    BOOL ranIt = NO;
