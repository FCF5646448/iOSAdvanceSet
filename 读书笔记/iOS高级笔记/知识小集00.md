...
##### 队列和任务:
* 队列分为串行队列和并发队列：串行队列是指同一时间，队列中只能执行一个任务；并发队列运行多个任务在同一时间同时进行；iOS中主队列mian是一种串行队列，全局队列global是一种并发队列；串行队列只对应一个线程，并发队列对应多个线程。
* 任务有同步执行还是异步执行之分：同步执行会把当前任务添加到队列中，等任务执行完成，线程才会返回执行下一个任务；异步执行会把当前任务加到队列中，线程无须等到任务执行完成就可以立即返回；所以同步执行不会开启新的线程，异步执行会开启新的线程。
##### 4种线程组合：
* 同步串行：同步不开启新线程，串行队列让任务一个一个执行。所以它实际不会产生多线程运算；
* 同步并发：并发队列虽然可以并发执行，但是同步执行不开启线程，也就相当于任务只在一个线程中执行，所以也不构成多线程运算；
* 异步串行：异步执行虽然会开线程，但是串行队列中任务一个个执行，导致最终也是只有一个线程在执行，所以也不构成多线程运算；
...
// 分析：首先队列里的任何任务都是一个一个执行完成的，也就是说外部任务执行完成时，内部任务肯定也得执行完成。其次这里使用串行队列，它只对应一个线程。外部任务的执行只对应当前线程，内部任务的执行也只对应当前线程。外部任务执行的过程中，它会一直占用当前线程，而内部任务想要执行也得独占当前线程。所以就导致内部任务没有线程而无法执行，外部任务因为内部任务没有执行完成导致自身无法完成，也就无法释放线程。所以就导致了死锁！ 
SerialQueue.sync{
    SerialQueue.async{
    }
}
//分析：同上面的分析类似。外部任务要执行完成得要内部任务执行完成，内部任务使用异步执行，线程无需等待执行完会立即返回，所以虽然都是在当前线程中执行，但是因为内部的任务不会长占线程，所以是可以执行完成的。内部任务执行完成后，外部任务也就可以执行完成
