...
2、进入SDWebImageManager的loadImageWithURL:options:progress:completed:会先使用SDImageCache的queryCacheOperationForKey:options:done:从缓存查找图片;
3、SDImageCache的queryCacheOperationForKey:options:done:会优先调用imageFromMemoryCacheForKey在缓存中进行图片查找，如果没有找到，则进行磁盘查找，然后将查找结果回调。
4、如果没有在本地找到，则会根据需要判断是否下载图片。下载图片是使用SDWebImageDownloader的downloadImageWithURL:options:progress:completed:进行下载。
5、下载完成后，将图片进行缓存，然后返回。
```
...
* 第4步中：图片的下载是由SDWebImageDownloader单例执行，负责图片的异步下载， 操作在一个NSOperationQueue的并发队列里，队列中的最大并发量是6；使用信号量来确保downloadQueue的线程安全；使用字典来存储下载operation，以便cancel；然后设置超时,默认是15.0,
* 第5步：
* 其他：
  ...
  * NSCache：内存缓存使用的是NSCache的子类。NSCache是类似NDDictionary的容器，它是线程安全的，所以在任意线程进行添加、删除都不需要加锁；
