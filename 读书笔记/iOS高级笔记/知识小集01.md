...
* 内存被爆 和 主线程超时：
  ...
  * 内存消耗监测：
  首先可以使用内存映射来获取线程堆栈信息。调用task_info API获取task_vm_info_data_t数据，获取它的phys_footprint。然后内存不足之前是会回调didReceiveMemoryWarning的函数的，回调这个函数距离被杀掉应该有大概6秒的时间，所以也可以在这里动态获取内存上限值，然后将情形上报。你也可以在这个函数里释放掉一些内存。
  ```
  //当前app消耗的内存
  ...
  ```
    * 监测卡顿：
      卡顿检测还是使用runloop的原理。我们可以在主线程下的common模式下创建一个runloopObserver。然后创建一个子线程专门用来监控主线程的runloop状态。如果监听到进入睡眠前KCFRunLoopBeforeSources状态或者唤醒后的KCFRunLoopAfterWaiting状态在设置的时间阈值内没有变化，即可判定为卡顿，然后获取到堆栈信息，进一步分析到哪个方法的执行时间过长。
  ```
	//主线程创建观察者
...
dispatch_async(dispatch_get_global_queue(0, 0), ^{
  ...
  ```
这个3秒就是Watch Dog的时间阈值。所以如果在这个检测里收集到卡顿日志，那么它就有可能就被crash掉了。
